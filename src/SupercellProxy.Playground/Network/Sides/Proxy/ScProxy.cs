...
public class ScProxy(ProxyConfiguration configuration)
{
    ...
    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    using (client)
                        await RunClientAsync(client, cancellationToken);
                }
    ...
    private async Task RunClientAsync(TcpClient tcpClient, CancellationToken cancellationToken = default)
    {
        await using var client = await BuildClientAsync(tcpClient, configuration.UpstreamHost, configuration.UpstreamPort, cancellationToken);

        try
        {
            await client.RunAsync(client.CancellationTokenSource.Token);
        }
        ...
        {
            await client.CompletionTask;
        }
    }

    private static async Task<ScProxyClient> BuildClientAsync(TcpClient client, string upstreamHost, int upstreamPort, CancellationToken cancellationToken = default)
    {
        ...
        await upstream.ConnectAsync(upstreamHost, upstreamPort, linkedCancellationTokenSource.Token);

        var clientStream = new SupercellStream(client.GetStream());
        var serverStream = new SupercellStream(upstream.GetStream());

        return new ScProxyClient(client, upstream, clientStream, serverStream, new EventBus(), linkedCancellationTokenSource);
    }
}
