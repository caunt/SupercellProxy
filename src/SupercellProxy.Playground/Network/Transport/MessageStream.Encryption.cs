...
internal sealed partial class MessageStream
{
    ...
    public async ValueTask SetupEncryptionAsync(
        Side with,
        Memory<byte> sessionKey,
    ...
    )
    {
        ...
        _encryption = new Encryption(
            ...
            localPrivateKey: with is Side.Server
                ? RandomNumberGenerator.GetBytes(count: 32)
                : ScProxy.StandardPrivateKey,
            remotePublicKey: with is Side.Server
                ? await HayDayApi.GetServerPublicKeyAsync(cancellationToken).ConfigureAwait(false)
                : default(Memory<byte>)
        );
    }
    ...
    private static MemoryStream DecryptServerboundHandshake(Encryption encryption, byte[] payload)
    {
        ...
        if (!receivedSessionKey.Span.SequenceEqual(encryption.SessionKey.Span))
            throw new InvalidOperationException(
                "Received session key does not match the expected session key."
            );
        ...
        if (
            ...
            && plaintextMemory.Span[^PromonPadSize..].SequenceEqual(stackalloc byte[PromonPadSize])
        )
            payloadLength -= PromonPadSize;
    ...
    private static MemoryStream EncryptServerboundHandshake(Encryption encryption, byte[] payload)
    {
        ...
        return new MemoryStream(
            [.. encryption.LocalPublicKey.Span, .. ciphertext],
            writable: false
        );
    }
    ...
    private MemoryStream Decrypt(MemoryStream memoryStream)
    {
        ...
        var payload =
            ...
            && streamBuffer.Array is { } buffer
            && streamBuffer.Offset is 0
            && streamBuffer.Count == buffer.Length
                ? buffer
        ...
        if (!_encryption.SharedKey.IsEmpty)
        {
            ...
            return new MemoryStream(
                NaClV3Crypto.SecretBoxOpen(
                    payload,
                    _encryption.ReceiveNonce.Span,
                    _encryption.SharedKey.Span
                ),
                writable: false
            );
        }
    ...
    private MemoryStream Encrypt(MemoryStream memoryStream)
    {
        ...
        if (!_encryption.SharedKey.IsEmpty)
        {
            ...
            return new MemoryStream(
                NaClV3Crypto.SecretBox(
                    payload,
                    _encryption.SendNonce.Span,
                    _encryption.SharedKey.Span
                ),
                writable: false
            );
    ...
    private sealed class Encryption(
        Side with,
        Memory<byte> localPrivateKey,
        Memory<byte> sessionKey,
        Memory<byte> remotePublicKey = default
    )
    {
        ...
        public Memory<byte> LocalPublicKey { get; init; } =
            NaClV3Crypto.CryptoScalarMultBase(localPrivateKey.Span);
