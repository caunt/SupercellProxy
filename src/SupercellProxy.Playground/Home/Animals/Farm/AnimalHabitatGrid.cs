...
{
    private readonly AnimalHabitatGridCell[] cells;

    private AnimalHabitatGrid(int width, int height)
    {
        Width = width;
        Height = height;
        cells = new AnimalHabitatGridCell[checked(width * height)];

        for (var y = 0; y < height; y++)
        for (var x = 0; x < width; x++)
            cells[x + width * y] = new AnimalHabitatGridCell(x, y);
    }
    ...
    public AnimalHabitatGridCell? Select(GameRandom random, bool requireUnoccupied)
    {
        ...
        var start = random.NextInt(cells.Length);

        for (var offset = 0; offset < cells.Length; offset++)
        {
            var cell = cells[(start + offset) % cells.Length];
    ...
    {
        return x >= 0 && y >= 0 && x < Width && y < Height && cells[x + Width * y].Walkable;
    }
