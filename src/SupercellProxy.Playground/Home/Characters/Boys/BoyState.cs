...
)
{
    ...
    {
        if (!dataTableResolver.TryGetTableId(GameAssetFiles.Boy, out var boyTableId))
            throw new InvalidOperationException(
                $"{GameAssetFiles.Boy} is not registered as a native data table."
            );
    ...
    {
        return checked(tilePosition * GameObjectState.TileSize + GameObjectState.TileCenter);
    }
}
