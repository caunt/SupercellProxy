...
)
{
    ...
    {
        const string constructionBuildingsFile = "data/construction_buildings.csv";

        if (
            !dataTableResolver.TryGetTableId(
                constructionBuildingsFile,
                out var constructionBuildingTableId
            )
        )
            throw new InvalidOperationException(
                $"{constructionBuildingsFile} is not registered as a native data table."
            );
